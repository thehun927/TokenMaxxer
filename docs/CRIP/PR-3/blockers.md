...
## 2026-08-10 — wave-10 second release-gate fix (oracle re-review f85a9d4)
- [bug-fix] Blocker 1: durable conflict reconstruction can mark a previously selected automated row `still_valid=false` without removing it from `authorities`; `resolveDecisionAuthorities` now purges any authority that is `still_valid !== true` OR whose normalized topic is under a `conflicting-human-foundational` quarantine after the second pass — src/memory/decision-authority.ts:352 — a conflicting-human-foundational topic reports ZERO automated authorities (reader invariant).
- [bug-fix] Blocker 2: duplicate-ID migration assigned a fresh `randomUUID()` to every duplicate on each pure read-only load, so repaired IDs were not stable across `decisions`/`recall_decision` and the transaction's `bypassCache: true` re-read; `repairDuplicateDecisionIds` is now a pure function of the input bytes — src/memory/migrate.ts:143.
- [design-decision] Deterministic-ID form: the canonical winner (oldest timestamp asc, then lexical ID asc) KEEPS its old shared ID so lineage references (`superseded_by`/`conflicts_with`/`derived_from_decision_id`) naturally resolve to it; every non-winner gets a fixed-length 36-char digest (SHA-256 of old ID + stable group ordinal + `tokenmaxxer-pr3-migrate:v1:` separator) via `derivedDecisionId` — src/memory/migrate.ts:241. Digest form chosen over a sequential `${oldId}-dup-${ordinal}` because it is always within `MAX_IDENTIFIER` regardless of legacy ID length/characters and is immune to overlong-input truncation problems.
- [bug-fix] Concern A: `quarantinedHumanConflicts()` trusted the `human_conflict_quarantined` marker on any row; it now requires `isHumanTrustRow(row)` so a schema-valid heuristic row carrying the flag cannot freeze a topic into human quarantine — src/memory/merge.ts:197.
- [design-decision] Concern B: pre-wave-9 v3 allowed arbitrary-length decision IDs; an overlong ID (> `MAX_IDENTIFIER`) is now repaired deterministically with the same digest (ordinal 0) during load instead of failing v3 validation — src/memory/migrate.ts (repairDuplicateDecisionIds overlong branch).
- [design-decision] Demotion rule pinned: when ANY row in a duplicate group carries `human_review`, the ENTIRE group (canonical winner included) is demoted to `foundational=false` + `foundational_requested=true` + human claims downgraded to `extractor=legacy`/`confidence=legacy` + `human_review` cleared, so a deterministic re-read observes consistent state and no duplicate is silently treated as the confirmed review target.
- [test-update] test/memory/migrate.test.ts wave-9 test "assigns fresh UUIDs to every duplicate" updated to the deterministic semantics (winner keeps the old shared ID; non-winners receive bounded derived IDs); the old assertion that all repaired IDs were fresh UUID v4 encoded the exact behavior the oracle rejected.
- [test-gap] Regression tests added: 4 Blocker-1 tests (zero authorities / queryDecisions / getProjectState / mergeDecisions-persist-reload) + 1 Concern-A test in test/memory/decision-authority.test.ts; 4 migrate determinism/demotion/lineage/overlong tests in test/memory/migrate.test.ts; 1 store transaction-re-read test in test/memory/store.test.ts; 1 recall transaction-re-read test in test/tools/recall.test.ts; 1 end-to-end CLI decisions→promote test in test/cli.test.ts. Full suite: 34 files, 418 tests (was 406).
- [design-decision] No `randomUUID()` remains in the read-only migration path (`src/memory/migrate.ts`); the write path (merge/decision-review/project-lock/fs) still uses it for minting new IDs under the lock, which is outside the pure-read contract.
