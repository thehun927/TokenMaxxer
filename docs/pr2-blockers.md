...
## 2026-08-10 — wave-7 release-gate lock fixes (oracle verdict Block)
- [bug] Blocker 1: stale recovery could quarantine a newly acquired live replacement lock because `quarantineStaleLock` renamed the canonical path without binding to the classified owner nonce — src/memory/project-lock.ts — fixed with an identity-preserving recovery claim + owner revalidation before quarantine.
- [design-decision] Recovery claim shape: a unique `.recovery-claim-<pid>-<nonce>` file created with `writeFile(..., { flag: "wx" })` INSIDE the canonical `.state-lock`; after creation the current `owner.json` is re-read and must still match the classified nonce AND still be dead (PID ESRCH) — src/memory/project-lock.ts:acquireRecoveryClaim — closes the classification→quarantine window; claim is a soft mutual-exclusion marker (documented in `quarantineStaleLock` JSDoc).
- [design-decision] `retireRecoveryClaim` is best-effort `rm(claimPath, { force: true })`; the claim is only a mutual-exclusion marker during recovery — src/memory/project-lock.ts.
- [bug] Blocker 2: `rename(candidate, .state-lock)` could replace an existing EMPTY destination on POSIX, stealing an unknown/missing-metadata lock — src/memory/project-lock.ts:acquireOnce — replaced with true create-if-absent `mkdir(lockDir, { recursive: false })` + `writeFile(owner.json)`; the brief empty-publication interval is safe because contenders classify it as unknown-owner and retry.
- [design-decision] Release is now retire-then-delete: after nonce verification, atomically `rename(lockDir, .state-lock.released.<nonce>.<8-hex>)` then best-effort recursive `rm` of ONLY the unique retired path; a failed rename (someone else acquired) returns false and never deletes the canonical path — src/memory/project-lock.ts:buildHandle.
- [test-gap] `crash-with-lock` fixture now genuinely leaves a dead lock: child acquires and waits forever (keep-alive interval) inside the callback; parent SIGKILLs it; dead PID detected via `process.kill(pid, 0)` → ESRCH — test/fixtures/project-lock-worker.ts.
- [test-gap] Added `acquire-and-hold` fixture command (acquire + wait-forever, released cleanly by the parent) for tests needing a real held lock — test/fixtures/project-lock-worker.ts.
- [test-gap] Added `waitForClassificationBarrier` option to `withProjectLock` (test-only hook) that pauses a contender immediately after classification until `<path>.release` exists — src/memory/project-lock.ts — used to deterministically force the replacement-between-classification-and-quarantine race.
- [test-gap] New regression tests in test/memory/project-lock.test.ts: SIGKILL dead-owner recovery; empty-canonical-lock not stolen (tryAcquire times out, withProjectLock throws ProjectLockTimeoutError, lock unchanged); release/acquire handoff (A's cleanup cannot touch B); release cleanup operates on a retired path; ABA-safe concurrent recovery; and the critical replacement-between-classification-and-quarantine test proving C1 cannot move/delete live replacement B.
- [test-gap] transaction.test.ts two-child same-project test is now barrier-driven: child A holds the lock (`hold-write`), child B waits at a pre-mutation barrier (`barrier-write`) so B is guaranteed blocked on the lock while A holds it; final revision N+2 with both facts — test/fixtures/transaction-worker.ts.
- [test-gap] recall.test.ts promotion test is now barrier-driven: child holds the lock (`hold-write`) before the parent drives `_recallPromote`, proving actual contention rather than sequential execution — test/memory/recall.test.ts.
- [flakiness] 5x adversarial run of project-lock/transaction/recall/p0-a-reliability/writer-llm/writer/store: 122 tests passed on all 5 runs; no flakiness. Full suite: 30 files, 307 tests green; `npx tsc --noEmit` exit 0.
